...
def allocate_paths(paths: Sequence[str], forms: Sequence[str], seed: int | None, max_per_form: int | None = None) -> Tuple[Dict[str, List[str]], int, List[str]]:
    if seed is not None:
        random.seed(seed)
    pool = list(paths)
    random.shuffle(pool)
    ...
    # Apply max_per_form limit if specified
    if max_per_form is not None and per_form > max_per_form:
        per_form = max_per_form
...
def generate_for_form(form: str, paths: Sequence[str], out_dir: str, seed: int | None, max_per_form: int | None = None) -> Tuple[str, int]:
    ...
    # Apply max_per_form limit if specified
    if max_per_form is not None and target > max_per_form:
        target = max_per_form
...
def main():
    ...
    par.add_argument("--max_per_form", type=int, help="Maximum number of pieces to generate per form")
    args = par.parse_args()
    ...
    slices, per_form, leftover = allocate_paths(all_midis, args.forms, args.seed, args.max_per_form)
    ...
    with ProcessPoolExecutor(max_workers=min(args.max_workers, len(args.forms))) as exe:
        futs = [exe.submit(generate_for_form, f, slices[f], args.output_folder, args.seed, args.max_per_form)
                for f in args.forms]
